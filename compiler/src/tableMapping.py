                ...
                for newAddr in queryStrBinAddrList:
                    ...
                    if matching == len(re.findall('x',oldAddr)):
                        printVerbose(verbose,
                                        f'count = {count2} | orig Search Query = {oldAddr} | '
                                        f'new Search Query = {newAddr} | matching ratio = {matching} |')
                        logging.info('count = %d | orig Search Query = %s | new Search Query = %s | matching ratio = %f |', 
                                        count2, oldAddr, newAddr, matching)
    ...
    def mapTCAMtoSRAM(self,verbose,debug):
        ...
        if len(tcamRowList) == len(sramColList):
            for (queryStr, pma, qsCol) in itertools.zip_longest(sramAddrList, tcamRowList, sramColList):
                ...
                if verbose or debug:
                    logging.info('SRAM table cell [%d, %d] | Value = %s -> %s', 
                                rowIndex, colIndex, oldSramTableEntry, sramDF.iat[rowIndex,colIndex])
