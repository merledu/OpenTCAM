...
def main():
    ...
    if arg.tcamConfig:
        ...
        if (memLength == 64) and (memWidth%7 == 0):
            ...
            logging.info('Created TCAM memory RTL dir: {:s}'.format(tcamRtlConfigDirPath))
            # * copy sky130 block to required config dir
            shutil.copy(pathSram1kb, tcamRtlConfigDirPath)
            logging.info('Copied file "{:s}" to dir "{:s}"'.format(pathSram1kb, tcamRtlConfigDirPath))
            print('Copied rtl file "{:s}" to dir "{:s}"'.format(os.path.basename(pathSram1kb), tcamRtlConfigDirPath))
            ...
            print('Created rtl file: "{:s}"'.format(rtlFilePath))
            if arg.debug:
            ...
            logging.info('"INVALID" TCAM memory wrapper config. tcam_[64]x[7*N] != {:s}'.format(arg.tcamConfig))
            sys.exit('"INVALID" TCAM memory wrapper config. tcam_[64]x[7*N] != {:s}'.format(arg.tcamConfig))
